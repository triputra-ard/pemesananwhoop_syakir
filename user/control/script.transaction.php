    ...
    if ($file_size1 < 3500640) {
      if (!is_dir($moveditem)) {
        ...
        if (move_uploaded_file($file_temp1, $moveditem.$filename1)) {
          ...
          $sql = "UPDATE whoop_transaksi SET foto_transaksi = '$directory/$filename1'
          ,status_transaksi = 'PROSES' Where id_reservasi = '$trx_id'";

          $query = mysqli_query($db,$sql);
          if ($query) {
            $update = "UPDATE whoop_reservasi SET status_reservasi = 'MENUNGGU_KONFIRMASI' where id_reservasi = '$trx_id'";
